...
`ifndef FORMAL
        begin : init_mem_from_file
            // Manual loader (one byte per line hex) to avoid simulator-specific
            // $readmemh range warnings for small init files.
            integer fd;
            integer addr;
            integer rc;
            reg [1023:0] line;
            ...
            if (init_file == "") begin
                // Optional override for simulation convenience
                void'($value$plusargs("ROM=%s", init_file));
            end

            if (init_file != "") begin
                fd = $fopen(init_file, "r");
                if (fd == 0) begin
                    $display("WARNING: could not open ROM init file=%s", init_file);
                end else begin
                    addr = 0;
                    while (!$feof(fd) && (addr < MEM_SIZE)) begin
                        line = "";
                        rc = $fgets(line, fd);
                        if (rc != 0) begin
...
`ifndef FORMAL
            mem_we = 1'b1;
            mem_waddr = imm_addr16;
            mem_wdata = regs[op0[1:0]];
`endif
            pc_next = pc + 16'd3;
