...
module cnorm_prove_formal;
    ...
    // A_ovf_shift3_nb1_fire : SHIFT=11, DR[nb+3]!=DR[nb+1] → OVF must be 1.
    //   Checks the DR[11] clause (line 125) fires when DR has mismatched bit 11.
    //   Kills M016 (arithmetic_step_flip: DR[nb+1]→DR[nb+0] in line 125, drops
    //   the DR[11] check and duplicates DR[10]; OVF stays 0 when DR[13]!=DR[11]
    //   but DR[13]==DR[10]).
    always @(posedge CLK) begin
        if (!$initstate && $past(ED) && !$past(START) && $past(SHIFT) == 2'b11 &&
            $past(DR[NB+3]) != $past(DR[NB+1]))
            A_ovf_shift3_nb1_fire: assert(OVF == 1'b1);
    end
