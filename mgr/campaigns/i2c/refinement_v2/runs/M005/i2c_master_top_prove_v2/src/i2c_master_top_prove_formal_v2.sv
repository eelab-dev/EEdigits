`default_nettype none

//=============================================================================
// Formal prove harness for i2c_master_top_formal (refinement v2)
//=============================================================================
module i2c_master_top_prove_formal_v2;
  localparam int unsigned MAX_CMD_CYCLES = 512;
  localparam int unsigned WDOG_W = 10;

  (* gclk *) logic wb_clk_i;

  (* anyseq *) logic wb_rst_i;
  (* anyseq *) logic arst_i;
  (* anyseq *) logic [2:0] wb_adr_i;
  (* anyseq *) logic [7:0] wb_dat_i;
  wire [7:0] wb_dat_o;
  (* anyseq *) logic wb_we_i;
  (* anyseq *) logic wb_stb_i;
  (* anyseq *) logic wb_cyc_i;
  wire wb_ack_o;
  wire wb_inta_o;

  wire scl_pad_i;
  wire scl_pad_o;
  wire scl_padoen_o;
  wire sda_pad_i;
  wire sda_pad_o;
  wire sda_padoen_o;

  (* anyseq *) logic ext_scl_low;
  (* anyseq *) logic ext_sda_low;

  assign scl_pad_i = (scl_padoen_o == 1'b0) ? 1'b0 : (ext_scl_low ? 1'b0 : 1'b1);
  assign sda_pad_i = (sda_padoen_o == 1'b0) ? 1'b0 : (ext_sda_low ? 1'b0 : 1'b1);

  i2c_master_top_formal dut (
      .wb_clk_i(wb_clk_i),
      .wb_rst_i(wb_rst_i),
      .arst_i(arst_i),
      .wb_adr_i(wb_adr_i),
      .wb_dat_i(wb_dat_i),
      .wb_dat_o(wb_dat_o),
      .wb_we_i(wb_we_i),
      .wb_stb_i(wb_stb_i),
      .wb_cyc_i(wb_cyc_i),
      .wb_ack_o(wb_ack_o),
      .wb_inta_o(wb_inta_o),
      .scl_pad_i(scl_pad_i),
      .scl_pad_o(scl_pad_o),
      .scl_padoen_o(scl_padoen_o),
      .sda_pad_i(sda_pad_i),
      .sda_pad_o(sda_pad_o),
      .sda_padoen_o(sda_padoen_o)
  );

  wire wb_wacc = wb_we_i && wb_ack_o;

  always @(posedge wb_clk_i) begin
    if ($initstate) begin
      assume(wb_rst_i);
      assume(arst_i == 1'b0);
    end else begin
      assume(!wb_rst_i);
      assume(arst_i == 1'b1);
    end
  end

  always @(posedge wb_clk_i) begin
    if (!$initstate && !wb_rst_i && (arst_i == 1'b1)) begin
      assume(ext_scl_low == 1'b0);
      assume(ext_sda_low == 1'b0);
    end
  end

  wire wb_cmd_req = wb_stb_i && wb_cyc_i && wb_we_i &&
                    (wb_adr_i == 3'b100) && (wb_dat_i[7:4] != 4'h0) &&
                    dut.core_en && (dut.prer <= 16'd4);

  logic cmd_watchdog_active;
  logic [WDOG_W-1:0] cmd_watchdog;

  logic tip_watchdog_active;
  logic [WDOG_W-1:0] tip_watchdog;
  logic tip_q;

  always @(posedge wb_clk_i) begin
    tip_q <= dut.tip;

    if ($initstate || wb_rst_i || (arst_i == 1'b0)) begin
      cmd_watchdog_active <= 1'b0;
      tip_watchdog_active <= 1'b0;
      cmd_watchdog <= '0;
      tip_watchdog <= '0;
    end else begin
      if (wb_cmd_req) begin
        cmd_watchdog_active <= 1'b1;
        cmd_watchdog <= MAX_CMD_CYCLES[WDOG_W-1:0];
      end else if (cmd_watchdog_active && dut.done) begin
        cmd_watchdog_active <= 1'b0;
      end else if (cmd_watchdog_active) begin
        assert(cmd_watchdog != '0);
        cmd_watchdog <= cmd_watchdog - {{(WDOG_W-1){1'b0}}, 1'b1};
      end

      if (!tip_q && dut.tip && (dut.prer <= 16'd4)) begin
        tip_watchdog_active <= 1'b1;
        tip_watchdog <= MAX_CMD_CYCLES[WDOG_W-1:0];
      end else if (tip_watchdog_active && dut.done && !dut.tip) begin
        tip_watchdog_active <= 1'b0;
      end else if (tip_watchdog_active) begin
        assert(tip_watchdog != '0);
        tip_watchdog <= tip_watchdog - {{(WDOG_W-1){1'b0}}, 1'b1};
      end
    end
  end

  always @(posedge wb_clk_i) begin
    assert(scl_pad_o == 1'b0);
    assert(sda_pad_o == 1'b0);

    assert((scl_padoen_o != 1'b0) || (scl_pad_i == 1'b0));
    assert((sda_padoen_o != 1'b0) || (sda_pad_i == 1'b0));

    if (!$initstate) begin
      assert(!wb_ack_o || $past(wb_cyc_i && wb_stb_i && !wb_ack_o));
    end

    if ((arst_i == 1'b0) || wb_rst_i) begin
      assert(wb_inta_o == 1'b0);
    end

    if (!$initstate && $past(!wb_rst_i) && ($past(arst_i) == 1'b1)) begin
      if ($past(wb_wacc && (wb_adr_i == 3'b000))) assert(dut.prer[7:0] == $past(wb_dat_i));
      if ($past(wb_wacc && (wb_adr_i == 3'b001))) assert(dut.prer[15:8] == $past(wb_dat_i));
      if ($past(wb_wacc && (wb_adr_i == 3'b010))) assert(dut.ctr == $past(wb_dat_i));
      if ($past(wb_wacc && (wb_adr_i == 3'b011))) assert(dut.txr == $past(wb_dat_i));
      if ($past(wb_wacc && (wb_adr_i == 3'b100) && dut.core_en)) assert(dut.cr == $past(wb_dat_i));

      if ($past(!wb_wacc && !(dut.done || dut.i2c_al))) begin
        assert(dut.cr[7:4] == $past(dut.cr[7:4]));
      end

      assert(dut.tip == (dut.rd || dut.wr));
      assert(wb_inta_o == (dut.irq_flag && dut.ien));

      if ($past(dut.irq_flag && dut.ien)) begin
        assert(wb_inta_o);
      end
    end
  end

  always @(posedge wb_clk_i) begin
    if (!$initstate && !wb_rst_i && (arst_i == 1'b1)) begin
      cover(wb_ack_o);
      cover(wb_wacc && (wb_adr_i == 3'b010));
      cover(wb_wacc && (wb_adr_i == 3'b011));
      cover(wb_wacc && (wb_adr_i == 3'b100));
      cover(wb_cmd_req);
      cover($rose(dut.tip));
      cover(dut.done);
      cover(dut.irq_flag);
      cover(wb_inta_o == 1'b1);
    end
  end
endmodule

`default_nettype wire
